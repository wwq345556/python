...
courseCount = "select * from yxy_school_info_tb where `del` = 1 and review_status=2 and status=1 and disable=0 and display = 0 and type in (2,4) and topic = 117 and keywords <> '格灵视频'"
courseCount = cursor.execute(courseCount)
mainsort = 0

for num in range(courseCount):
    try:
        courseMainSql = "select * from yxy_school_info_tb where `del` = 1 and review_status=2 and status=1 and disable=0 and display = 0 and type in (2,4) and topic = 117 and keywords <> '格灵视频' order by order_id desc limit %d,%d" % (num, 1)
        ...
        if grade_id in senior:
            ...
            cursor3.execute(mainInsertSql, (
                ...
                str(is_del), str(is_review), str(creator), str(create_time), str(hits), str(description)))
            conn3.commit()
        # print( str(grade_id), str(subject_id), str(edition_id), str(subject_id), str(chapter_id),
        #       str(knowledge_id))
