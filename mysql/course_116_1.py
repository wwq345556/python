...
courseCount = "select * from yxy_school_info_tb where `del` = 1 and review_status=2 and status=1 and disable=0 and type in (2,4) and topic = 115"
courseCount = cursor.execute(courseCount)
mainsort = 0

for num in range(courseCount):
    # try:
        courseMainSql = "select * from yxy_school_info_tb where `del` = 1 and review_status=2 and status=1 and disable=0 and type in (2,4) and topic = 115 order by order_id desc limit %d,%d" % (num, 1)
