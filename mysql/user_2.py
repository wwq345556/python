...
#连接tifen_mall
conn3 = pymysql.connect(host="192.168.1.176",port=3002, user="root",password="i226CtSmDMn71dlyIqdZ0pI",database="tifen_mall",charset="utf8")
    ...
    try:
        ...
        for member_id in MemberIdSet:
            ...
            # enable 1=>0 其余=>1
                ...
                # 反查学习点
                pointSql = "select point_total,point_balance from user_basic where user_id = %s"
                cursor3.execute(pointSql,str(id))
                point = cursor3.fetchone()
                if point:
                    credits_total = point[0]
                    credits_balance = point[1]
                else:
                    credits_total = 0
                    credits_balance = 0
                ...
                str(id), str(school), str(member_id), str(nickname), str(realname), str(member_pwd), str(sex),str(credits_total),str(credits_balance),
                str(is_teacher), str(subjects), str(plate_id), str('0'), str(photo), str(phone_no), str(phone_bind), str(disabled),
